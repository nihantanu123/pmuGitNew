...
namespace PMU.PMUServices.PMUDal
{
    public static class PMULoginDal
    {
        ...
        /// <summary>
        ///  Check whether user exist with given username and password
        /// </summary>
        /// <param name="userName"></param>
        /// <param name="password"></param>
        /// <returns></returns>
        public static int IsUserAuthorized(string userName, string password)
        {
            int IsAuthorized = 0;
            Database db = null;
            ...
            try
            {
                ...
                {
                    db.AddInParameter(objCommand, DalConstants.StoredProcedures.User.UserDetailsParameters.userName, DbType.String, userName);
                    db.AddInParameter(objCommand, DalConstants.StoredProcedures.User.UserDetailsParameters.passWord, DbType.String, password);
                    int.TryParse(db.ExecuteScalar(objCommand).ToString(), out IsAuthorized);
                }
            }
            catch(Exception ex)
            {
